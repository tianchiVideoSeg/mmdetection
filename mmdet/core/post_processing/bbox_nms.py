...
def diou(box_a, box_b):
    ...
    out = inter / union - D ** 0.9
    return out


def cluster_diounms(multi_bboxes,
                    multi_scores,
                    iou_threshold):
    scores, idx = multi_scores.sort(descending=True)
    scores = scores.view(1, -1)
    bboxes = multi_bboxes[idx, :].view(1, -1, 4)

    iou = diou(bboxes, bboxes).triu_(diagonal=1)
    B = iou
    for i in range(200):
        A = B
        maxA, _ = A.max(dim=1)
    ...
    return torch.cat([bboxes, scores.unsqueeze(1)], -1), torch.where(keep == 1)[1]
...
def multiclass_nms(multi_bboxes,
    ...
    if nms_cfg["type"] == "diounms":
        dets, keep = cluster_diounms(bboxes, scores, nms_cfg["iou_threshold"])
    else:
